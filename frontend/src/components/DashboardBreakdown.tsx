...
export const DashboardBreakdown = ({ data, dimension, loading, onDimensionChange }: Props) => {
  ...
  const leader = [...data.desglose.filas].sort((a, b) => b.ventas_eur - a.ventas_eur)[0];
  const growth = [...data.desglose.filas].sort((a, b) => b.variacion_eur - a.variacion_eur)[0];
  const decline = [...data.desglose.filas].sort((a, b) => a.variacion_eur - b.variacion_eur)[0];
    ...
    <section className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {[
          ...
          { title: 'Mayor crecimiento', row: growth, value: growth ? `+${formatEUR(growth.variacion_eur)}` : '—', icon: TrendingUp, tone: 'text-emerald-600 bg-emerald-500/10' },
          { title: 'Mayor caída', row: decline, value: decline ? `−${formatEUR(Math.abs(decline.variacion_eur))}` : '—', icon: TrendingDown, tone: 'text-red-600 bg-red-500/10' },
